...
const profilePictureStyle = cva("rounded-full w-10 h-10");
const authorName = cva("text-white font-semibold");
const tweetContent = cva("text-gray-300", {
  variants: {
    censored: {
      true: "italic",
      false: "",
...
const lockedText = cva("text-red-400 text-sm");
const mediaContainer = cva("mt-2 flex flex-wrap gap-2");
const editTextArea = cva("w-full bg-transparent text-white outline-none resize-none placeholder-gray-400");
const fileLabel = cva("cursor-pointer bg-gray-700 p-2 rounded");
const replyFormContainer = cva("mt-2");
const replyTextArea = cva("w-full bg-gray-800 text-white outline-none resize-none p-2");
const errorText = cva("text-red-500 mt-2");

const replyContainer = cva("mt-4 pl-8 border-l border-gray-600");
const replyItem = cva("mb-2");
const replyHeader = cva("flex items-center gap-2");
const replyImage = cva("w-6 h-6 rounded-full");
const replyAuthor = cva("text-white font-semibold");
    ...
    <article className={tweetContainer()}>
      <div className={headerContainer()}>
        <Link to={`/profile/${author}`}>
          <img src={profilePicture} alt={`${author} profile`} className={profilePictureStyle()} />
        </Link>
        <div className={contentContainer()}>
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            {isOwner && !isEditing && (
              <button onClick={() => setIsEditing(true)} className={controlButton({ intent: "primary" })}>
            ...
            {isOwner && (
              <button onClick={handleDelete} className={controlButton({ intent: "danger" })}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
            ...
            <div onClick={handleRetweet} className={clickableIcon()}>
            ...
            {isOwner &&
              (isLockedState ? (
                <button onClick={handleUnlock} className={controlButton({ intent: "warning" })}>
                ...
                <button onClick={handleLock} className={controlButton({ intent: "warning" })}>
                  <svg width="20" height="24" viewBox="0 0 20 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clipPath="url(#clip0_410_1374)">
                      <path
                        d="M3.5 6.5V10H2C1.46957 10 0.960859 10.2107 0.585786 10.5858C0.210714 10.9609 0 11.4696 0 12L0 22C0 22.5304 0.210714 23.0391 0.585786 23.4142C0.960859 23.7893 1.46957 24 2 24H18C18.5304 24 19.0391 23.7893 19.4142 23.4142C19.7893 23.0391 20 22.5304 20 22V12C20 11.4696 19.7893 10.9609 19.4142 10.5858C19.0391 10.2107 18.5304 10 18 10H16.5V6.5C16.5 4.77609 15.8152 3.12279 14.5962 1.90478C13.3772 0.685796 11.7239 0 10 0C8.27609 0 6.62279 0.685796 5.40381 1.90478C4.18482 3.12377 3.5 4.77707 3.5 6.50098V10.001H2C1.46957 10.001 0.960859 10.2117 0.585786 10.5868C0.210714 10.9618 0 11.4705 0 12.001L0 22ZM8 15.5C8.00007 15.1535 8.09016 14.813 8.26143 14.5118C8.43269 14.2106 8.67927 13.9591 8.97699 13.7819C9.27472 13.6046 9.61337 13.5078 9.95978 13.5008C10.3062 13.4939 10.6485 13.577 10.9531 13.7421C11.2577 13.9072 11.5142 14.1486 11.6974 14.4427C11.8807 14.7368 11.9844 15.0734 11.9984 15.4196C12.0124 15.7658 11.9362 16.1097 11.7773 16.4176C11.6184 16.7255 11.3823 16.9868 11.092 17.176L11.084 17.181C11.084 17.181 11.279 18.361 11.499 19.751V19.752C11.4987 19.9506 11.4197 20.1409 11.2793 20.2813C11.1389 20.4217 10.9486 20.5007 10.75 20.501H9.248C9.04943 20.5007 8.85908 20.4217 8.71867 20.2813C8.57826 20.1409 8.49926 19.9506 8.499 19.752V19.751L8.914 17.181C8.63309 16.9998 8.40207 16.7511 8.24205 16.4577C8.08203 16.1642 7.99813 15.8353 7.998 15.501L8 15.5Z"
                        fill="black"
                      />
                    </g>
                    <defs>
          ...
          {localReplies.length > 0 && !isLockedState && (
            <div className={replyContainer()}>
              {localReplies.map((reply, index) => (
                <div key={reply.id || index} className={replyItem()}>
                  <div className={replyHeader()}>
                    <img src={reply.profilePicture} alt={reply.username} className={replyImage()} />
                    <span className={replyAuthor()}>{reply.username}</span>
                    <span className={replyTimestamp()}>{reply.createdAt}</span>
                  </div>
                  <p className={replyContentStyle()}>{reply.content}</p>
                </div>
              ))}
            </div>
          )}
